...
run - Quicly queue and run jobs from the command line.

=head1 DESCRIPTION

L<run> tries to run a set of commands using different handlers. It is primarily useful for running commands
...
a cluster and the qsub command exists, L<run> will create submission scripts and submit them to the scheduler.
...
If all other handlers fail, L<run> will fallback to just running each command in serial, one after the other.
This turns out to still be useful, because if you have a set of simulations that you need to perform, you can
use L<run> to automatically run each simulation after one has completed.
...
Note that L<run> does not do load ballancing. Each handler is given the list of commands to run, and they are responsible
...
dry run. don't execute hanlder commands, just print them to stdout
...
prepend CMD to all commands befor running

=item C<-d DEL>

use DEL to delimite commands (default '$arg_delim')
...
send standard output to files. note that this is only meaningful for handlers that would normally send stdout to the screen.
...
treat all arguments as the names of already existing wrapper scripts and run dirrectly without generating new wrapper scripts.
